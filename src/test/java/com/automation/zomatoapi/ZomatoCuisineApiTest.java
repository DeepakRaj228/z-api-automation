...
public class ZomatoCuisineApiTest extends BaseSetup {

    @Test
    public void testGetCuisinesWithValidCityId(){
        int city_id = 7;
        Response resp = RestAssured.given()
                .spec(getBaseRequestSpecification()).queryParam("city_id",city_id).get("/cuisines");
    ...
    public void testGetCuisinesWithLatOnly(){
        ...
        Response resp = RestAssured.given()
                .spec(getBaseRequestSpecification()).queryParam("lat",latitude).get("/cuisines");
        JSONObject jsonobj = new JSONObject(resp.body().asString());
        Assert.assertEquals(resp.getStatusCode(),400);
        Assert.assertEquals(jsonobj.get("message").toString(),"Invalid coordinates / city_id");
    }
    ...
    public void testGetCuisinesWithLonOnly(){
        ...
        Response resp = RestAssured.given()
                .spec(getBaseRequestSpecification()).queryParam("lon",longitude).get("/cuisines");
        JSONObject jsonobj = new JSONObject(resp.body().asString());
        Assert.assertEquals(resp.getStatusCode(),400);
        Assert.assertEquals(jsonobj.get("message").toString(),"Invalid coordinates / city_id");
    }
    ...
    public void testGetCuisinesWithLatLonOnly(){
        ...
        Response resp = RestAssured.given()
                .spec(getBaseRequestSpecification()).queryParam("lat",latitude).
                        queryParam("lon",longitude).get("/cuisines");
        Assert.assertEquals(resp.getStatusCode(),200);
    }
